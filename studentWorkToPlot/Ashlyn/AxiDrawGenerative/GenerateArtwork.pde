...
void makeRectangle(float x, float y) {
  ...
  x = x + map(randomizer.getPseudoRandom(),10,1,minDist,maxDist);
  y = y + map(randomizer.getPseudoRandom(),10,1,minDist,maxDist);
  
  // use these value for perlin randomness in position of rectangle
   //x = x + map(randomizer.getPerlinRandom(),0,0,minDist,maxDist);
   //y = y + map(randomizer.getPerlinRandom(),0,0,minDist,maxDist);
  
  float firstX = x;
  float firstY = y;
  
  // change these to control size of rectangles
  float minLength = 10; //10
  float maxLength = 40; //50
...
void makeShapes(float startX, float startY, int shapeCount) {
  ...
  while(shapeNumber < shapeCount) 
  {
    ...
    makeTriangle(startX, startY, 80);
    makeCircle(startX+random(-40,40), startY+random(-40,40),10);
    makeCircle(startX, startY, 10);
...
void dropPen() 
{
   ToDoList = (PVector[]) append(ToDoList, new PVector(-31, 0)); //Command 31 (lower pen) new PVector(-31, 0)
}

void raisePen() 
{
   ToDoList = (PVector[]) append(ToDoList, new PVector(-30, 0)); //Command 30 (raise pen)new PVector(-30, 0)
}
